...
import { Notify } from 'vant';
  ...
  export default {
    ...
    methods: {
       onFailed(errorInfo) {  //表单提交错误的信息打印
       ...
       sendReg() {  
         ...
         if(this.password1 && this.mobel && this.textName && this.getMessage){
            if(this.password1 === this.password2){
              ...
              console.log(44);
               this.$axios.post(`http://localhost:3000/register/cellphone?phone=${this.mobel}&password=${this.password1}&captcha=${this.getMessage}&nickname=${this.textName}`).then(res=>{
                 if(res.status ==200){
                   console.log("注册成功");
        ...
        verify(){
                ...
                this.$axios.post(`http://localhost:3000/captcha/verify?phone=${this.mobel}&captcha=${this.getMessage}`)
                      .then(res=>{
                      console.log(res);
                      return res;
    ...
    onClickLeft() {
      ...
      //请求链接
            if(this.mobel){
                this.$axios.post(`http://localhost:3000/captcha/sent?phone=${this.mobel}`).then(res=>{
                console.log(res);
